...
for year in range(2023, EYR + 1):
    return_growth_rate.loc[str(year), 'Returns'] = return_growth_rate.loc[str(2022), 'Returns']
return_growth_rate.Returns.index = index

# read SOI estimates for 2008+
soi_estimates = pd.read_csv(os.path.join(CUR_PATH, "SOI_estimates_test.csv"),
                            index_col=0)
...
for i in range(SOI_YR, EYR):  # SOI Estimates loop
    ...
    Interest_1 = return_projection.INTS1[i]*cbo_growth_rates.INTS[i+1]
    Interest_2 = return_projection.INTS2[i]*cbo_growth_rates.INTS[i+1]
    Interest_3 = return_projection.INTS3[i]*cbo_growth_rates.INTS[i+1]
    Interest_4 = return_projection.INTS4[i]*cbo_growth_rates.INTS[i+1]
    Interest_5 = return_projection.INTS5[i]*cbo_growth_rates.INTS[i+1]
    Interest_6 = return_projection.INTS6[i]*cbo_growth_rates.INTS[i+1]
    Interest_7 = return_projection.INTS7[i]*cbo_growth_rates.INTS[i+1]
    Interest_8 = return_projection.INTS8[i]*cbo_growth_rates.INTS[i+1]
    Interest_9 = return_projection.INTS9[i]*cbo_growth_rates.INTS[i+1]
    Interest_10 = return_projection.INTS10[i]*cbo_growth_rates.INTS[i+1]
    Interest_11 = return_projection.INTS11[i]*cbo_growth_rates.INTS[i+1]
    Interest_12 = return_projection.INTS12[i]*cbo_growth_rates.INTS[i+1]
    ...
    current_year = pd.DataFrame([Single, Joint, HH,
                                 ...
                                 Interest_11, Interest_12, DIVS, SCHCI, SCHCL,
...
data = Stage_II_targets[Stage_II_targets.columns[19+11:31+11]].sum(axis=1)
total_wage = pd.DataFrame(data, columns=['AWAGE'])

data = Stage_II_targets[Stage_II_targets.columns[8:8+12]].sum(axis=1)
INTS_val = pd.DataFrame(data, columns=['INTS'])
...
Stage_I_factors['AINTS'] = INTS_val/INTS_val.INTS[SYR]
...
rename = {
    ...
    'INTS1': 'Taxable Interest Income: Zero or Less',
    'INTS2': 'Taxable Interest Income: $1 Less Than $10,000',
    'INTS3': 'Taxable Interest Income: $10,000 Less Than $20,000',
    'INTS4': 'Taxable Interest Income: $20,000 Less Than $30,000',
    'INTS5': 'Taxable Interest Income: $30,000 Less Than $40,000',
    'INTS6': 'Taxable Interest Income: $40,000 Less Than $50,000',
    'INTS7': 'Taxable Interest Income: $50,000 Less Than $75,000',
    'INTS8': 'Taxable Interest Income: $75,000 Less Than $100,000',
    'INTS9': 'Taxable Interest Income: $100,000 Less Than $200,000',
    'INTS10': 'Taxable Interest Income: $200,000 Less Than $500,000',
    'INTS11': 'Taxable Interest Income: $500,000 Less Than $1 Million',
    'INTS12': 'Taxable Interest Income: $1 Million and Over',
